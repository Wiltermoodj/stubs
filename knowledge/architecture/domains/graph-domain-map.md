...
| :-------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------- | :------------------------- |
| [`src/graph/engine.ts`](../../src/graph/engine.ts) / [`engine.ts.md`](../../src/graph/engine.ts.md) | Central `GraphEngine` class managing SQLite queries, schema tables (`graph_nodes`, `graph_edges`), transactions, and state updates | `sql.js`, `better-sqlite3` |
| [`src/graph/extractor.ts`](../../src/graph/extractor.ts)                                            | AST & multi-language symbol, call, import, and OKF link extraction for TS, JS, Python, Rust, Go                                    | `typescript`, `parser/okf` |
| [`src/graph/topology.ts`](../../src/graph/topology.ts)                                              | `TopologyEngine` managing blast radius traversal, BFS shortest path, Tarjan SCC cycle detection, and God Node centrality           | `extractor.ts`             |
